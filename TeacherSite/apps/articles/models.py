...
class VPR(models.Model):
        ...
        descr=models.CharField('описание', blank=True,max_length=500)
        category=models.ForeignKey('articles.Category',null=True,on_delete=models.PROTECT,verbose_name='Предмет')
        file=models.FileField(upload_to='vpr/')
        direct=models.ForeignKey('articles.Direction',on_delete=models.PROTECT,verbose_name='Направление')
        type=models.ForeignKey('articles.VPRtype',null=True,blank=True,on_delete=models.PROTECT,verbose_name='Тип материала')
        pub_date=models.DateTimeField('дата публикации',blank=True,auto_now_add=True)
        def __str__(self):
            return self.name
...
class VPRtype(models.Model):
    name=models.CharField('тип материала',max_length=70)

    def __str__(self):
        return self.name

    class Meta:
        verbose_name='Тип материала ВПР'
        verbose_name_plural='Типы материала ВПР'
...
class AnswerRecieved(models.Model):
     result=models.ForeignKey(Test_result,on_delete=models.CASCADE)
     answer=models.ForeignKey(Answer,on_delete=models.CASCADE)
     correct_choices=[('Y','Да'),('N','Нет')]
     isright=models.CharField(max_length=3,choices=correct_choices)
